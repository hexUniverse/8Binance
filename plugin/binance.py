...
def binance(bot, update):
    ...
    extrac = re.findall(pattern, update.message.text_html)
    ### EXTRACT URL ###
    for url in extrac:
        update.message.text = url
        if validators.url(update.message.text) != True:
            return
        pattern = '(binance|exhangecenter|marketrelease|zendesk|dex2019|dextestnet|dexsolutions|dexsupports|exchange)'
